...
控制台和战报属于公共入口，当前放在 `game/cmd/web`。`game/core` 放跨玩法公共微服务，`game/features` 放具体玩法微服务，`game/cmd` 只负责命令、按钮和 HTTP 触发，`game/app.py` 是唯一组合根。当前已有数据、战斗、资源池、世界和数据库五个公共微服务；人物、构筑、行路、物品、炼药和炼器等业务仍需按 JSON 契约逐个重建。
