...
def load_presentation(data: JsonDataService) -> tuple[SectCopy, tuple[Mapping[str, str], ...]]:
    ...
    buttons = tuple(
        MappingProxyType(
            {key: str(_mapping(raw, "宗门按钮[]").get(key) or "").strip() for key in ("页面", "编号", "名称", "命令", "行为", "样式")}
        )
        for raw in raw_buttons
    )
    if len({button["编号"] for button in buttons}) != len(buttons):
        raise JsonDataError("宗门按钮编号不能重复")
    if any(button["页面"] not in {"未加入", "待处理邀请", "宗主", "成员"} for button in buttons):
        raise JsonDataError("宗门按钮使用了未知页面")
    return SectCopy(text), buttons


def actions(buttons: tuple[Mapping[str, str], ...], page: str) -> tuple[SectAction, ...]:
    return tuple(
        SectAction(button["编号"], button["名称"], button["命令"], button["行为"], button["样式"])
