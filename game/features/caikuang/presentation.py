...
def load_presentation(
...
) -> tuple[OreGatheringCopy, tuple[Mapping[str, str], ...]]:
    raw_text = data.dataset("采矿展示").get("文本")
    if not isinstance(raw_text, Mapping):
        raise JsonDataError("采矿展示缺少文本.json")
    ...
    rows = data.dataset("采矿按钮").get("按钮")
    if not isinstance(rows, Sequence) or isinstance(rows, (str, bytes)):
        raise JsonDataError("采矿按钮必须是字典列表")
    buttons = tuple(
        MappingProxyType(
            {
                key: str(_mapping(raw, "采矿按钮[]").get(key) or "").strip()
