...
def current_session(request: Request) -> ConsoleSession:
    ...
    if session is None:
        raise HTTPException(status_code=401, detail="天道管理台尚未登录或会话已过期")
    return session
...
def write_session(
    ...
    if not service.auth.verify_csrf(session, x_csrf_token):
        raise HTTPException(status_code=403, detail="天道管理台操作校验失败")
    origin = str(request.headers.get("origin") or "").rstrip("/")
    expected = str(request.base_url).rstrip("/")
    if origin and origin != expected:
        raise HTTPException(status_code=403, detail="天道管理台拒绝跨站操作")
    return session
...
async def console_page() -> HTMLResponse:
    if not service.auth.configured:
        return HTMLResponse(
            "<h1>天道管理台尚未启用</h1><p>请在 .env 中设置 WEB_CONSOLE_PASSWORD。</p>",
            status_code=503,
        )
    return HTMLResponse(
...
async def login(
    ...
    if not service.auth.configured:
        raise HTTPException(status_code=503, detail="天道管理台尚未配置密码")
